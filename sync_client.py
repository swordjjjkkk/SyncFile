...
def CompareDatabase(old,new):
    ...
    for (key,value) in new.items():
        # key=key.replace(config['client']['path'],'',1)
        if old.get(key)!=None:
            if value!=old[key]:
                res[key]={"b64file":EncodeFile(config['client']['path']+key),"flag":1}
    for (key,value) in new.items():
        # key=key.replace(config['client']['path'],'',1
        if old.get(key)==None:
            res[key]={"b64file":EncodeFile(config['client']['path']+key),"flag":2}
...
def EventLoop(socketid):
    ...
    while True:
        ...
        time.sleep(config['client']['syncinterval'])
